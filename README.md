...
**Option A — Committed file (recommended, no token).** Export your Airtable table to
CSV, save it as `data/tools.csv`, and commit it. The build reads that file — **no
Airtable token, no API permissions, nothing to fail on auth.** A sample `data/tools.csv`
is included so you can see the exact format; just replace it with your real export.
To update the site, drop in a fresh export and redeploy. This is the fastest, most
reliable way to launch.

**Option B — Live API.** Set `AIRTABLE_TOKEN` + `AIRTABLE_BASE_ID` and the build pulls
straight from Airtable on every build. More automated, but requires a Personal Access
Token whose account has the base added under Access.

The build auto-detects: **if `AIRTABLE_TOKEN` is set it uses the API; otherwise it uses
`data/tools.csv`.** You can start with Option A today and switch to Option B later just
by adding the token — no code change.

### How to export from Airtable (Option A)
Open your table (or a view) → click the view name / the **…** menu → **Download CSV**.
Save it into this project as `data/tools.csv`, overwriting the sample. Done.

Either way, the **free build ships only the count of use cases**, never the premium text.
...
3. **Environment variables** — **only needed for Option B (live API).** If you're using
   Option A (`data/tools.csv`), skip this entirely; there's nothing to configure and the
   build can't fail on auth. For Option B, add:
   - `AIRTABLE_TOKEN`
   - `AIRTABLE_BASE_ID`
   - `AIRTABLE_TABLE`
